import './index.scss'
import { useEffect, useState } from 'react'
import React from 'react'
import AnimatedLetters from '../Animated_Letters'
import Vivus from 'vivus';

export default function Resume() {
    const [letterClass, setLetterClass] = useState('text-animate')
  
    useEffect(() => {
      return setTimeout(() => {
        setLetterClass('text-animate-hover')
      }, 3000)
    }, [])

    useEffect(() => {
        const vivusInstance = new Vivus('drawme', { type: 'sync', duration: 500, delay: 0 });
        return () => {
          vivusInstance.destroy(); // Cleanup to avoid memory leaks
        };
      }, []);

    return (
        <div className='container resume-page'>
            <div className="text-zone">
                <h1>
                    <AnimatedLetters
                    letterClass={letterClass}
                    strArray={['M', 'y', ' ', 'R', 'e', 's', 'u', 'm', 'e']}
                    idx={15}
                    />
                </h1>
                <div>
                    <p>
                        Explore my professional journey and accomplishments by downloading my resume below. 
                        I'm excited to share my career highlights and qualifications with you!
                    </p>
                </div>
                <div>
                    <a href="../../assets/Resume/SatwikPandeyResume.pdf" download='Satwik_Resume.pdf'>
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 512 512"
                            // style="enable-background:new 0 0 512 512"
                            // xml:space="preserve"
                            // fill="none"
                            stroke='#ffd700'
                            strokeWidth='2'
                            width="100pt"
                            height="200pt"
                            id='drawme'
                        >
                            <path d="M431.917 45.295h-28.146v328.493l-89.921 89.92H105.231v26.074c0 8.049 6.524 14.573 14.573 14.573h312.113c8.05 0 14.574-6.523 14.574-14.573V59.868c0-8.048-6.524-14.573-14.574-14.573z"/>
                            <path d="M129.29 489.781v-26.073h-24.058v26.074c0 8.049 6.524 14.573 14.573 14.573h24.059c-8.049-.001-14.574-6.525-14.574-14.574zM423.33 388.225c3.07-3.013 4.5-8.243 4.5-12.842V45.295h-24.06v328.493l-89.921 89.92h33.054l76.427-75.483z"/>
                            <path d="M406.77 373.788V22.22c0-8.049-6.524-14.575-14.574-14.575H80.083c-8.049 0-14.574 6.525-14.574 14.575v429.913c0 8.048 6.523 14.573 14.574 14.573H313.85l92.92-92.918z"/>
                            <path d="M90.495 452.133V22.22c0-8.049 6.523-14.575 14.574-14.575H80.083c-8.049 0-14.574 6.525-14.574 14.575v429.913c0 8.048 6.523 14.573 14.574 14.573h24.987c-8.051 0-14.575-6.524-14.575-14.573z"/>
                            <path d="M234.456 189.952c0 8.049-6.523 14.575-14.573 14.575h-95.941c-8.05 0-14.574-6.525-14.574-14.575V64.865c0-8.047 6.523-14.574 14.574-14.574h95.941c8.049 0 14.573 6.526 14.573 14.574v125.087z"/>
                            <path d="M174.606 156.353h-25.768c-8.049 0-14.573 6.524-14.573 14.574v33.599h25.767v-33.599c0-8.048 6.523-14.574 14.574-14.574z"/>
                            <path d="M134.637 189.952V64.865c0-8.047 6.523-14.574 14.575-14.574h-25.27c-8.05 0-14.574 6.526-14.574 14.574v125.088c0 8.049 6.523 14.575 14.574 14.575h25.27c-8.05-.001-14.575-6.527-14.575-14.576z"/>
                            <path d="M194.988 156.353h-46.15c-8.049 0-14.573 6.524-14.573 14.574v33.599h75.296v-33.599c0-8.048-6.525-14.574-14.573-14.574z"/>
                            <circle cx="171.91" cy="109.717" r="24.373"/>
                            <path d="M175.416 85.593c-14.851-2.128-27.877 9.388-27.877 24.12 0 13.462 10.912 24.374 24.373 24.374 6.623 0 12.627-2.646 17.02-6.934-19.966-2.873-27.923-27.497-13.516-41.56zM406.77 373.788H303.437c-8.049 0-14.574 6.525-14.574 14.573v78.346h24.987l92.92-92.919z"/>
                            <path d="M406.77 373.788h-78.347c-8.049 0-14.573 6.525-14.573 14.573v78.346l92.92-92.919z"/><path d="M219.862 211.997c12.177 0 22.082-9.907 22.082-22.083v-79.976c0-4.143-3.357-7.5-7.5-7.5a7.499 7.499 0 0 0-7.5 7.5v79.976c0 3.905-3.177 7.082-7.081 7.082h-2.828v-26.119c0-12.177-9.906-22.082-22.081-22.082h-46.177c-12.177 0-22.082 9.906-22.082 22.082v26.119h-2.828c-3.904 0-7.081-3.177-7.081-7.082V64.753c0-3.904 3.177-7.081 7.081-7.081h95.997c3.904 0 7.081 3.177 7.081 7.081v19.184c0 4.143 3.357 7.5 7.5 7.5s7.5-3.357 7.5-7.5V64.753c0-12.177-9.906-22.082-22.082-22.082h-95.997c-12.177 0-22.082 9.906-22.082 22.082v125.161c0 12.177 9.906 22.083 22.082 22.083h95.996zm-78.167-15.001v-26.119c0-3.904 3.177-7.081 7.081-7.081h46.177c3.904 0 7.08 3.177 7.08 7.081v26.119h-60.338z"/>
                            <path d="M139.974 109.628c0 17.584 14.306 31.89 31.889 31.89s31.889-14.306 31.889-31.89c0-17.583-14.306-31.889-31.889-31.889s-31.889 14.306-31.889 31.889zm48.776 0c0 9.313-7.575 16.889-16.888 16.889s-16.888-7.576-16.888-16.889 7.575-16.888 16.888-16.888 16.888 7.576 16.888 16.888zM147.053 245.529c0 4.143 3.357 7.5 7.5 7.5h208.485c4.143 0 7.5-3.357 7.5-7.5s-3.357-7.5-7.5-7.5H154.553a7.498 7.498 0 0 0-7.5 7.5zM363.038 282.543H154.553a7.499 7.499 0 0 0-7.5 7.5c0 4.143 3.357 7.5 7.5 7.5h208.485c4.143 0 7.5-3.357 7.5-7.5a7.5 7.5 0 0 0-7.5-7.5zM363.038 342.059c4.143 0 7.5-3.357 7.5-7.5s-3.357-7.5-7.5-7.5H154.553c-4.143 0-7.5 3.357-7.5 7.5s3.357 7.5 7.5 7.5h208.485zM125.712 238.028h-7.29a7.499 7.499 0 0 0-7.5 7.5c0 4.143 3.357 7.5 7.5 7.5h7.29c4.143 0 7.5-3.357 7.5-7.5a7.5 7.5 0 0 0-7.5-7.5zM125.712 282.543h-7.29a7.499 7.499 0 0 0-7.5 7.5c0 4.143 3.357 7.5 7.5 7.5h7.29c4.143 0 7.5-3.357 7.5-7.5s-3.357-7.5-7.5-7.5zM125.712 327.058h-7.29c-4.143 0-7.5 3.357-7.5 7.5s3.357 7.5 7.5 7.5h7.29c4.143 0 7.5-3.357 7.5-7.5s-3.357-7.5-7.5-7.5z"/>
                            <path d="M446.602 245.447c4.143 0 7.5-3.357 7.5-7.5V59.753c0-12.177-9.906-22.082-22.082-22.082h-17.662V22.083C414.358 9.907 404.452 0 392.276 0H79.98C67.803 0 57.897 9.907 57.897 22.083v430.165c0 12.177 9.906 22.082 22.082 22.082h17.663v15.588c0 12.176 9.906 22.081 22.082 22.081H432.02c12.177 0 22.082-9.906 22.082-22.081v-225.97c0-4.143-3.357-7.5-7.5-7.5s-7.5 3.357-7.5 7.5v225.971c0 3.904-3.177 7.08-7.081 7.08H119.725c-3.904 0-7.081-3.176-7.081-7.08v-15.588c211.303 0 202.45.477 205.037-1.042a7.112 7.112 0 0 0 1.506-1.154c98.2-98.199 93.162-93.074 93.774-93.931a7.387 7.387 0 0 0 1.385-4.094c.003-.085.012-.168.012-.253V52.671h17.662c3.904 0 7.081 3.177 7.081 7.081v178.194a7.502 7.502 0 0 0 7.501 7.501zM79.98 459.33c-3.904 0-7.081-3.177-7.081-7.081V22.083c0-3.905 3.177-7.082 7.081-7.082h312.296c3.904 0 7.081 3.177 7.081 7.082v344.273h-70.892c-12.177 0-22.082 9.906-22.082 22.081v70.892l-226.403.001zm241.403-10.608v-60.285c0-3.904 3.177-7.08 7.081-7.08h60.285c-7.14 7.141-60.226 60.227-67.366 67.365z"/>
                        </svg>                    
                    </a>
                </div>
            </div>
        </div>
  )
}
